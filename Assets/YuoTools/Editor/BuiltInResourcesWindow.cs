...
public class BuiltInResourcesWindow : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        {
            _scrollPos += Event.current.delta.y * 50;
            Event.current.Use();
        }
        ...
        if (Drawings == null)
        {
            ...
            else if (_showingIcons)
            {
                ...
                foreach (UnityEngine.Object oo in _objects)
                {
                    ...
                    float width = Mathf.Max(
                        GUI.skin.button.CalcSize(new GUIContent(texture.name)).x + 10,
                        iconWidth
                    ) + 8.0f;


                    float height = iconWidth + GUI.skin.button.CalcSize(new GUIContent(texture.name)).y + 8.0f;
                    ...
                    draw.Draw = () =>
                    {
                        try
                        {
                            ...
                            Rect textureRect = GUILayoutUtility.GetRect(iconWidth, iconHeight, iconWidth, iconHeight,
