...
namespace YuoTools.Extend.Helper.Network
{
    public class UdpHelper
    {
        ...
        public static void InitClient(string ip, int port, bool isReceive = true)
        {
            UDPClient = new UdpClient(ip, port);
            if (isReceive)
            {
                // UDPClient.Client.Bind(new System.Net.IPEndPoint(System.Net.IPAddress.Parse(ip), port));
                UDPClient.BeginReceive(ClientReceive, null);
            }
        }
        ...
        {
            var ip = new System.Net.IPEndPoint(System.Net.IPAddress.Any, 0);
            var data = UDPClient.EndReceive(ar, ref ip);
            OnReceive?.Invoke(System.Text.Encoding.UTF8.GetString(data));
            UDPClient.BeginReceive(ClientReceive, null);
        }
