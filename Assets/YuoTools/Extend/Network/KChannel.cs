        ...
        public void HandleRecv(byte[] date, int offset, int length)
        {
            ...
            while (true)
            {
                ...
                if (this.needReadSplitCount > 0) // 说明消息分片了
                {
                    ...
                    int count = this.kcp.Receive(buffer.AsSpan((int)(this.readMemory.Length - this.needReadSplitCount),
                        n));
                ...
                else
                {
                    ...
                    if (n == 8)
                    {
                        ...
                        if (headInt == 0)
                        {
                            ...
                            {
                                Log.Error(
                                    $"kchannel read error3: {this.needReadSplitCount} {this.LocalConn} {this.RemoteConn}");
        ...
        private void KcpSend(MemoryBuffer memoryStream)
        {
            ...
            else
            {
                ...
                while (alreadySendCount < count)
                {
                    ...
                    this.kcp.Send(memoryStream.GetBuffer()
                        .AsSpan((int)memoryStream.Position + alreadySendCount, sendCount));
