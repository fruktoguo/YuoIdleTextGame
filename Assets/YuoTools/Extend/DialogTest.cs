...
public class DialogTest : MonoBehaviour
{
    ...
    {
        if (!autoPlay) return;
        float length = m_TextComponent.text.Length;
        Schedule += Time.deltaTime * Speed;
        Schedule = Mathf.Repeat(Schedule, length);
        TextAnima(Schedule);
    }
    ...
    public IEnumerator Anima()
    {
        Schedule = 0;
        TextAnima(0);
        var length = m_TextComponent.text.Length;
    ...
    private void TextAnima(float schedule)
    {
        //刷新
        ...
        for (int i = 0; i < characterCount; i++)
        {
            ...
            for (int index = 0; index < 4; index++)
            {
                ...
                else
                {
                    ...
                    destinationVertices[vertexIndex + index] =
                        matrix.MultiplyPoint3x4(destinationVertices[vertexIndex + index]);
                    //矩阵计算完了再把开始减的加回去
